...
- **Feature Extractors**: Scripts that pull useful traits from URLs so the AI can learn.
  - `fast_url_feature_extraction.py`: Fast extractor that supports both URL-only datasets and URL + local HTML datasets.
...
3. Install the required libraries:
   ```bash
  pip install pandas numpy xgboost scikit-learn beautifulsoup4 lxml requests colorama tldextract python-whois
   ```
...
1. Configure `fast_url_feature_extraction.py` to point at either:
  - `dataset.csv` for URL-only extraction, or
  - `dataset-folder/dataset/data_dropped.csv` for URL + HTML extraction using files from `dataset-folder/dataset/`.
2. Run `python fast_url_feature_extraction.py` to generate `fast_url_features.csv`.
